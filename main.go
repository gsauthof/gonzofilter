...
func parse_args() *args {
    ...
    flag.BoolVar(&args.dump_words, "dump-words", false,
            "dump the parsed text after after word splitting for debugging purposes")
    flag.BoolVar(&args.dump_mark, "dump-mark", false,
            "dump the parsed text after after word splitting and marking for debugging purposes")
    flag.BoolVar(&args.dump_db, "dump-db", false, "dump the database")
    flag.BoolVar(&args.check, "check", false, "classify a message")
    flag.BoolVar(&args.passthrough, "pass", false, "classify a message and pass it through with the result stored in a X-gonzo header")
